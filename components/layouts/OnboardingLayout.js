...
const OnboardingLayout = ({ children, desc = "Monitor analytics for your cooperative and members.", img = "/login-img.png" }) => {
  return (
    <div className="min-h-screen w-screen relative flex">
      <section className="w-[48%] min-h-screen h-screen  bg-pv_primary flex items-center justify-center px-[2rem]">
      ...
      <section className="w-[62%] min-h-screen h-screen overflow-scroll scroll_hide bg-white flex items-center justify-center p-[2rem]">{children}</section>
    </div>
  );
};
