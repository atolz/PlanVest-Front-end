...
const StatCardWithBtn = ({ title, value, bgColor = "linear-gradient(263.32deg, #230B34 0.96%, #8B31CA 100%)", className, titleClassName, actionText = "Withdraw" }) => {
  return (
    <div style={{ background: bgColor }} className={`w-full p-[2.2rem] px-[2.2rem] rounded-secondary relative overflow-hidden flex flex-col justify-between min-w-[25rem] ${className}`}>
      <div>
      ...
      <Button className=" bg-white text-pv_primary max-w-[13.6rem]">{actionText}</Button>

      <div className=" absolute top-0 right-0 h-full">
        <img className="h-[8rem] w-auto scale-[1.4] absolute -left-[1.7rem] top-0" src="/card-path-2.png"></img>
